...
export type AssetCodeType = 'permanent' | 'consumable';
export type AssetCodeStatus = 'active' | 'redeemed' | 'revoked' | 'expired';

/**
 * Redemption codes for asset purchases
 * Generated by admin/payment system
 */
export const redemptionCodes = pgTable('redemption_codes', {
  ...
  /** Asset type: permanent or consumable */
  type: text('type').notNull().$type<AssetCodeType>(),
  /** Specific asset type (e.g., 'pro_tier', 'ai_tokens') */
  assetType: text('asset_type').notNull(),
  /** Value/balance for consumable assets */
  value: integer('value'),
  /** Metadata (JSON) - tier details, limits, etc. */
  metadata: jsonb('metadata').default({}).notNull(),
  /** Current status */
  status: text('status').notNull().$type<AssetCodeStatus>().default('active'),
  /** When created */
  createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
...
/**
 * Redemption receipts - identity-blind double-redemption prevention
 ...
 */
export const redemptionReceipts = pgTable('redemption_receipts', {
  /** Unique ID (ULID) */
  id: text('id').primaryKey(),
  /** HMAC-based receipt key for identity-blind tracking */
  receiptKey: text('receipt_key').notNull().unique(),
  /** FK to redemption code */
  codeId: text('code_id').notNull().references(() => redemptionCodes.id),
  /** When redeemed */
  redeemedAt: timestamp('redeemed_at', { withTimezone: true }).defaultNow().notNull(),
}, (table) => ({
  receiptKeyIdx: uniqueIndex('redemption_receipts_receipt_key_idx').on(table.receiptKey),
  codeIdIdx: index('redemption_receipts_code_id_idx').on(table.codeId),
}));
